...
- **Demo:** cambia `href="#"` en los dos enlaces con clase `demo-link` de `index.html`. Ambos tienen el comentario `REEMPLAZAR AQUÍ POR URL DE LA DEMO`. Actualiza también el mensaje inicial `#demo-feedback`. Con una URL real, los enlaces navegan normalmente.
...
- **Logo:** marca provisional en la cabecera y `favicon.svg`. Sustitúyelos por los activos oficiales.
...
- Documentación e historial con `<details>` nativo, utilizables sin JavaScript.
...
- Los botones de demo muestran un aviso accesible mientras no tengan una URL configurada.
