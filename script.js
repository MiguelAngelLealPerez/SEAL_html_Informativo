...
const revealElements = document.querySelectorAll('.section-heading, .problem-grid, .feature-grid, .workflow, .detail-grid, .ai-layout, .signature-layout, .audit-timeline, .security-layout, .architecture-layout, .development-timeline, .learning-grid, .users-grid, .team-grid, .demo-heading');
let revealObserver;
...
/* Enlace de demo: reemplazar href="#" en AMBOS botones de index.html.
   Cuando se asigne una URL real, se navegará normalmente sin cambios de JS. */
document.querySelectorAll('.demo-link').forEach(link => {
  link.addEventListener('click', event => {
    if (link.getAttribute('href') !== '#') return;
    event.preventDefault();
    const feedback = document.querySelector('#demo-feedback');
    feedback.textContent = 'La demo aún no tiene un enlace configurado. Visita nuestro stand para recorrer SEAL con el equipo.';
    feedback.classList.add('attention');
    feedback.setAttribute('tabindex', '-1');
    feedback.focus({ preventScroll: true });
    feedback.scrollIntoView({ behavior: motionPreference.matches ? 'instant' : 'smooth', block: 'center' });
  });
});
